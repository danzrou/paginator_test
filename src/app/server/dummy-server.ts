...
import { PagingRequest, PagingResponse } from '../pagination/pagination-service';
...
export class DummyServer {
  getUsers(pagingRequest: PagingRequest): Observable<PagingResponse<User>> {
    ...
    const us = this.getUserByPage(pageSize, requestedPage).filter(user => this.filterBySearchTerm(user, searchTerm));
    return of({
      metadata: {
